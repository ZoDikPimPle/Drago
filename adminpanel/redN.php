...
if (isset($_POST["UpdateFilm"])) {
    ...
    mysqli_query($dp, "UPDATE `фильмы` SET `Название` = '$UpdTitle', `Год_выпуска` = '$UpdYear', `Режиссер` = '$UpdDirector', `Описание` = '$UpdDescription' WHERE `Идентификатор` = '$film'");
...
function filterTable() {
  ...
  rows.forEach((row) => {
    if (row.querySelector("td")) { // Пропускаем заголовок таблицы
...

   {
    ...
    <form method="POST">
        </tr><td> <input  type= "text"  name= "add_film_name" placeholder="Введите название">  </td>
    ...
    while($film = mysqli_fetch_array($films)) {
        ...
        echo '<tr>
            ...
            <td>' . $film['Описание'] . '</td>
            <td><a href="' . $film['Постер'] . '" target="_blank" class="clickable-poster">' . $film['Постер'] . '</a></td>
    ...
    if (isset($_POST["add_film"])){
        $add_film_name = $_POST["add_film_name"];
        $add_film_director = $_POST["add_film_director"];
        $add_film_year = $_POST["add_film_year"];
        $add_film_description = $_POST["add_film_description"];
        $add_film_poster = $_POST["add_film_poster"];
